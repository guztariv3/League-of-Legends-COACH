...
- El gate del prototipo: `/api/health` queda abierto (lo usa Render), igual que `/api/desktop/claim` y `/api/desktop/scout`, que llevan su propia autenticación (código de un solo uso y token de dispositivo, ver `08-fase4.md`). Todo lo demás pide contraseña. Hay cabeceras de seguridad (CSP, `nosniff`, `frame-ancestors 'none'`) y la ruta SPA sirve la app sin tocar `/api/*`.
...
| Clave de Riot | **Personal API key aprobada** (no caduca). Solo para uso del propietario | Portal de Riot y sincronización sin errores |
...
Pendiente antes de abrirlo a más gente: rate limiting en los endpoints propios (ver `05-fase1.md`), una **Production key** (la Personal es solo para el propietario) y sustituir el gate por RSO (D-01).
...
| "Riot ha rechazado la clave de la API" | La clave de desarrollo caduca cada 24 h | Regenerarla en el portal de Riot y actualizar `RIOT_API_KEY` en Render (**Save, rebuild, and deploy**). **Resuelto:** ahora se usa la Personal API key, que no caduca |
