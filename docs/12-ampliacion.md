...
## F6a — Improve: Champion Pool, Matchups, LP y actividad (hecho)
Nueva sección **Improve** (barra de navegación; en el teléfono, sexto icono) con pestañas. **Champion pool** y **Matchups** (`championPool`, `matchupPool` en `packages/coach`): solo partidas analizables de la Grieta; partidas, victorias, KDA, CS/min y diferencias de oro y CS a los 15. El veredicto ("claramente ganando/perdiendo") solo aparece cuando el intervalo de Wilson al 95 % queda entero por encima o por debajo del 50 % con al menos 5 partidas; si no, lo dice. Los matchups se pueden filtrar por el campeón que jugabas. **LP**: gráfica de las fotos de rango de cada sincronización sobre una escalera continua (100 por división), con tabla; sin predicciones ni MMR, y avisa de que la historia empieza al vincular la cuenta (`/api/rank` devuelve `points`). **Activity**: calendario de 6 meses agrupado por día en la hora local del navegador (el servidor solo manda marcas de tiempo), con partidas en 30 días, días jugados, racha y el día de la semana en que más juegas.

## F6b — retos (hecho)
Pestaña **Challenges** (la primera de Improve). Un reto es un intento corto sobre una métrica de objetivo: "en 3 de tus próximas 5 partidas" o "3 partidas en 7 días". El objetivo lo fija el servidor al aceptar (percentil 60 en la buena dirección de tus últimas 20 partidas con la métrica, redondeado; solo se sugiere si lo alcanzaste en el 20–75 % de ellas), nunca el cliente ni otros jugadores. Solo cuentan las partidas empezadas después de aceptarlo, y una partida sin la métrica no cuenta ni a favor ni en contra. Se da por completado al tercer acierto y por no logrado cuando ya no se puede llegar (o se acaba la semana); se cierran solos al consultarlos. Máximo 3 a la vez y uno por métrica; se pueden abandonar. El foco guardado del Coach va primero en las sugerencias. Tabla `challenges` (migración 0006), `evaluateChallenge` y `suggestChallenges` en `packages/insights`.

## F8 — conocimiento de campeones con fuente (hecho, D-15)
El servidor descarga una vez al día el archivo combinado de Meraki Analytics (`lolstaticdata`, datos de la League of Legends Wiki bajo CC BY-SA 3.0), lo reduce a lo que la app muestra y lo guarda en memoria (`wikiSource` en `packages/knowledge`; solo en modo de datos reales; se precalienta al arrancar; espera como mucho 2–3 s en frío y conserva la última copia buena si una descarga falla). Página de campeón: **Playstyle** (posiciones, subclases, tipo de ataque, valoraciones 1–3 de la Wiki y fortalezas/debilidades tomadas tal cual de esas valoraciones) y, en cada habilidad, tipo de daño, objetivo, enfriamiento, coste y **valores por rango** con escalados al estilo de la Wiki. Lista de campeones: filtro por **posición**. La atribución (Wiki, licencia y Meraki) aparece en cada bloque que usa estos datos y en los Términos. **Combos y guías**: siguen sin fuente con licencia, así que la página lo dice y no se escriben a mano.
